...
const DeleteConfirmDialog = ({ garment, onClose, onSuccess }: Props) => {
  ...
  const handleConfirm = async () => {
    ...
    setError(null)
    try {
      await garmentService.remove(garment.id)
      onSuccess()
    } catch {
      setError('Nie udało się usunąć ubrania. Spróbuj ponownie.')
      setSubmitting(false)
    }
  }
      ...
      >
        ...
        <div className="px-7 py-6 flex flex-col gap-6">
          <p style={{ letterSpacing: '0.03em' }} className="text-sm text-[#4A4A47] leading-relaxed">
            ...
            <span className="font-medium text-[#1A1A18]">„{garment.name}"</span>?
            <span
